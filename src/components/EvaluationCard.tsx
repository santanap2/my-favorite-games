    ...
    <div className="flex items-center justify-between gap-6 sm:gap-2 w-full h-48 bg-white px-2 rounded shadow-md relative animation-opacity transition-all">
      {alreadyEvaluated && (
        <div className="flex items-center gap-1 absolute right-3 top-3 sm:right-0 sm:top-0">
          <span className="text-sm text-rose-400 font-semibold sm:hidden">
            Produto avaliado
          </span>
          <CheckCircle weight="fill" className="text-rose-400 text-2xl" />
        </div>
      )}
      <img
        src={image}
      ...
      <div className="flex flex-col w-full h-44 items-start justify-start gap-1">
        ...
        <div className="flex w-full h-full justify-between items-end">
          ...
          <button
            className="w-64 sm:w-full bg-rose-500 rounded px-2 py-2 text-white font-light text-base sm:text-sm shadow-md hover:bg-rose-600 transition-all"
            onClick={() => {
              alreadyEvaluated
                ? router.push(
