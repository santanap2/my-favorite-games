...
export default async function GameFilters({
  ...
  return (
    <div className="md:hidden w-56 h-full flex flex-col pb-6  justify-between items-center text-stone-300 z-20 px-2 border-r border-stone-800">
      <div className="flex flex-col items-center h-full justify-start w-full gap-10">
        <div className="flex flex-col w-full gap-1">
          <div className="w-full flex justify-between items-center pb-2 border-b border-stone-800 mb-4">
            ...
            <Link href="/home/1">
              <button className="text-left px-4 py-2 w-full h-fit text-xs font-semibold rounded-md hover:bg-stone-300 hover:bg-opacity-10 transition-all flex items-center justify-between space-x-3 hover:text-red-800">
          ...
          {categories.map(({ name, namePt }: ICategory) => (
            <Link key={name} href={handleFilters(name)}>
              <button
                className={`text-left text-stone-300 px-4 py-2 w-full text-sm font-semibold rounded-md hover:bg-stone-300 hover:bg-opacity-10 transition-all flex items-center justify-between ${queryParams.includes(name) && 'bg-stone-300 bg-opacity-30'} `}
              >
                <span>{namePt}</span>
