    ...
    <div className="rounded flex flex-col w-64 h-[500px] bg-white relative items-center justify-center shadow-md hover:shadow-lg hover:scale-105 cursor-pointer transition-all sm:w-full sm:h-96 xxl:w-52 xxl:h-96 animation-opacity">
      ...
      <Link href={`/game/${id}`} className="w-full">
        <div className="absolute top-72 w-full h-40 flex flex-col justify-between items-start px-[5%] py-2 sm:top-52 sm:px-2 sm:py-1 sm:h-32 xxl:top-52 xxl:h-32">
          ...
          <span className="text-xl font-bold text-violet-400 sm:text-lg">
            {`R$ ${priceToBRL(price)}`}
          </span>
        </div>
      </Link>
      ...
      <div className="absolute w-56 left-4 right-4 bottom-2 flex gap-[2.5%] sm:w-[90%] sm:left-[5%] sm:right-[5%] sm:bottom-[1.5%] xxl:w-[90%] xxl:left-[5%]">
        <button
          ...
          className="w-[72.5%] h-9 bg-violet-400 py-2 rounded text-sm uppercase font-bold  tracking-wide text-white hover:bg-violet-500 transition-all shadow-md  sm:text-xs"
        >
          Comprar
        </button>
        <button
          ...
          className="w-1/4 h-9 bg-violet-400 py-1 rounded text-sm uppercase flex items-center justify-center relative hover:bg-violet-500 transition-all shadow-md sm:text-xs "
        >
          <ShoppingCartSimple
