...
export default function EvaluationCard({
  ...
  return (
    <div className="flex items-center justify-between gap-6 w-full max-w-3xl h-36 bg-zinc-950 border border-zinc-900 p-2 rounded-md shadow-md">
      <img
        src={image}
        alt={name}
      ...
      <div className="flex flex-col w-full h-32 max-h-32 items-start justify-between gap-1">
        ...
        <div className="flex w-full justify-between items-end">
          ...
          <button
            className="flex xs:w-fit xs:text-xs w-48 justify-center rounded-md bg-zinc-300 px-3 py-1 sm:px-1 text-sm font-semibold leading-6 text-zinc-800 shadow-sm  transition-all"
            onClick={() => {
              alreadyEvaluated
                ? router.push(
