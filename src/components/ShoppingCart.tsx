          ...
          <div className="flex w-full justify-between pr-4 items-center">
            ...
            <button type="button" onClick={() => setShowCart(!showCart)}>
              <X
                size={28}
                weight="bold"
                className="text-zinc-800 hover:text-teal-400"
              />
            </button>
          </div>
            ...
            {data?.data.data.products.length > 0 ? (
              <div className="w-full flex flex-col items-center justify-center gap-3 mb-16">
                <button
                  ...
                  className="text-sm uppercase font-bold text-white py-2 bg-teal-400 rounded tracking-wide shadow-sm hover:shadow-lg w-4/5 sm:w-fit sm:px-4"
                >
                  {`Finalizar compra -  R$ ${
                ...
                <button
                  type="button"
                  className="uppercase tracking-wide underline text-xs font-light hover:text-teal-400"
                  onClick={() => {
                    setShowCart(false)
                    router.push('/home')
                  }}
